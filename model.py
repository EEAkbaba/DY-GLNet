...
import numpy as np 
from resnet_dcd.resnet_dcd import resnet50_dcd #---EEA
...
class SiameseNetwork(nn.Module):
    ...
    def make_back_bone(self,base_model):
        ...
        if base_model == 'resnet50':
            #model = torch.load('saved_models/rsscn7_2023-05-17_resnet50_91.74_baseline.pth')
            model = resnet50() #------EEA
            model.fc = nn.Linear(in_features=2048, out_features=self.num_classes, bias=True) #------EEA
            weights = torch.load('saved_models/rsscn7_2023-05-17_resnet50_91.74_baseline_sta.pth') #------EEA
            model.load_state_dict(weights)  #------EEA
            for param in model.parameters():
                if self.fixed:
                    param.requires_grad = False
        ...
        #------EEA
        if base_model == 'resnet50_dcd':
            #model = torch.load('saved_models/rsscn7_2023-05-17_resnet50_91.74_baseline.pth')
            model = resnet50_dcd() #------EEA
            model.fc = nn.Linear(in_features=2048, out_features=self.num_classes, bias=True) #------EEA
            weights = torch.load('saved_models/rsscn7_2023-05-21_resnet50_dcd_92.86_baseline_dyn_eniyi.pth') #------EEA
            model.load_state_dict(weights)  #------EEA
            for param in model.parameters():
                if self.fixed:
                    param.requires_grad = False
            model.classifier = nn.Linear(in_features=2048,out_features=self.out_features_dim)
            return model 
        #------EEA
