...
##-------------------------------EEA
from torchvision import models  #std resnet50 için
import cv2
import os
import numpy as np
...
class BasicBlock_dy(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu = nn.ReLU(inplace=True)     #nn.LeakyReLU
...
class Bottleneck_dy(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv_dy(planes, planes, 3, stride, 1) #ilk layerda 1, diğerlerinde 2
        self.bn2 = nn.BatchNorm2d(planes)
    ...
    def forward(self, x):
        ...
        if self.downsample is not None: #layer1de downsaple = None
            residual = self.downsample(x)

        out += residual ## burası boyutu değiştirmiyor
        out = self.relu(out)
        return out
...
class ResNet_dcd(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dropout=None): #cocoda num_classes=80
        self.inplanes = 64
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2) #orjinali
...
def resnet50_dcd(pretrained=False, **kwargs):
    ...
    #------EEA
    if pretrained:
        modelfile = torch.load('/content/drive/MyDrive/GLNet/resnet_dcd/resnet50_dcd.pth.tar')
        new_state_dict = dict()
        for key in modelfile['state_dict'].keys():
            new_key_name = key[7:]
            if new_key_name[:10] == 'classifier' and new_key_name[11] == '1':
                new_key_name = new_key_name[:11] + '0' + new_key_name[12:]
            new_state_dict[new_key_name] = modelfile["state_dict"][key]
        modelfile["state_dict"] = new_state_dict
        model.load_state_dict(modelfile['state_dict'])
    #----------------------------------------------------------EEA eski
    #mfile = torch.load('output/resnet50_dcd.pth.tar')

    ##new_state_dict = dict()

    # once mfile["state_dict"].keys()

    ##for key in mfile["state_dict"].keys():

    ##    new_key_name = key[7:]

    ##    new_state_dict[new_key_name] = mfile["state_dict"][key]

    ##print(new_state_dict["layer1.0.conv1.bn1.running_mean"].shape)

    ##mfile["state_dict"] = new_state_dict
    ##print(mfile["state_dict"]["layer1.0.conv1.bn1.running_mean"].shape)
    #if pretrained:
        #model.load_state_dict(model_zoo.load_url(model_urls['resnet50']), strict=False)
        #model.load_state_dict(torch.load('output/resnet50_dcd.pth.tar'))
        ##model.load_state_dict(mfile["state_dict"])
        #print("pretrained dinamik modeli yükledi")
        #model = torch.load('output/resnet50_dcd.pth.tar')
